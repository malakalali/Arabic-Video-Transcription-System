...
def burn_subtitles_into_video(video_path, srt_path, output_path):
    """Burns subtitles into video using ffmpeg with multiple fallback options."""
    try:
        # Escape the SRT path for ffmpeg
        escaped_srt_path = escape_for_ffmpeg_subtitles(srt_path)
        
        # Try multiple subtitle burning approaches
        approaches = [
            # Approach 1: Standard subtitle burning with Arial Unicode MS
            [
                'ffmpeg',
                '-i', video_path,
            ...
            # Approach 2: Use drawtext filter as fallback
            [
                'ffmpeg',
                '-i', video_path,
                '-vf', f"subtitles={escaped_srt_path}:force_style='Fontsize=20,Fontname=DejaVuSans'",
                '-c:a', 'copy',
                '-y',
                output_path
            ],
            # Approach 3: Simple subtitle burning without font specification
            [
                'ffmpeg',
                '-i', video_path,
        ...
        for i, command in enumerate(approaches):
            try:
                logger.info(f"Trying subtitle burning approach {i+1}: {' '.join(command)}")
                result = subprocess.run(command, check=True, capture_output=True, text=True, timeout=300)
                logger.info(f"Subtitle burning successful with approach {i+1}")
                return True
            except subprocess.CalledProcessError as e:
                ...
                logger.warning(f"Approach {i+1} timed out")
                continue
        
        # If all approaches fail, try to create a video without burning subtitles
        logger.warning("All subtitle burning approaches failed, creating video without burned subtitles")
        command = [
            'ffmpeg',
            '-i', video_path,
            '-c:v', 'copy',
        ...
        result = subprocess.run(command, check=True, capture_output=True, text=True)
        logger.info("Created video without burned subtitles as fallback")
        return True
...
def process_video():
    try:
        ...
        model_size = request.form.get('model_size', 'medium')  # Default to medium for better speed/accuracy balance
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                return jsonify({'error': f'Transcription failed: {transcribe_error}'}), 500
            ...
            if not burn_subtitles_into_video(video_path, srt_path, output_path):
                logger.warning("Subtitle burning failed, but continuing with SRT file")
                # If subtitle burning fails, just copy the original video
                try:
                    shutil.copy2(video_path, output_path)
                    logger.info("Copied original video as fallback")
                except Exception as copy_error:
                    logger.error(f"Failed to copy video: {copy_error}")
